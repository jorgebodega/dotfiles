...
link_rofi_config() {
    local src_dir=".config/rofi"
    local abs_src_dir="$(readlink -f $src_dir)"
    local target_dir="$HOME/.config/rofi"
    local backup_dir="$HOME/.config/rofi.backup"
    ...
    if [[ -e "$target_dir" || -L "$target_dir" ]]; then
        echo "Backing up existing rofi config to $backup_dir" >&2
