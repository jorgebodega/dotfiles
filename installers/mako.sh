...
link_mako_config() {
    local src_dir=".config/mako"
    local abs_src_dir="$(readlink -f $src_dir)"
    local target_dir="$HOME/.config/mako"
    local backup_dir="$HOME/.config/mako.backup"
    ...
    if [[ -e "$target_dir" || -L "$target_dir" ]]; then
        echo "Backing up existing mako config to $backup_dir" >&2
