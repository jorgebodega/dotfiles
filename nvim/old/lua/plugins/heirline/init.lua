return {
  ...
  config = function(_, opts)
    ...
    -- Take new Catppuccin palette
    vim.api.nvim_create_autocmd('ColorScheme', {
      pattern = '*',
      group = augroup,
      callback = function()
        local updated_colors = require('catppuccin.palettes').get_palette()

        heirline.load_colors(updated_colors)
      end,
    })
    ...
    --   local HeirlineInactive = astronvim.get_hlgroup('HeirlineInactive', { fg = nil }).fg or astronvim.status.hl.lualine_mode('inactive', C.grey_7)
    --   local HeirlineNormal = astronvim.get_hlgroup('HeirlineNormal', { fg = nil }).fg or astronvim.status.hl.lualine_mode('normal', C.blue)
    --   local HeirlineInsert = astronvim.get_hlgroup('HeirlineInsert', { fg = nil }).fg or astronvim.status.hl.lualine_mode('insert', C.green)
    --   local HeirlineVisual = astronvim.get_hlgroup('HeirlineVisual', { fg = nil }).fg or astronvim.status.hl.lualine_mode('visual', C.purple)
    --   local HeirlineReplace = astronvim.get_hlgroup('HeirlineReplace', { fg = nil }).fg or astronvim.status.hl.lualine_mode('replace', C.red_1)
    --   local HeirlineCommand = astronvim.get_hlgroup('HeirlineCommand', { fg = nil }).fg or astronvim.status.hl.lualine_mode('command', C.yellow_1)
    --   local HeirlineTerminal = astronvim.get_hlgroup('HeirlineTerminal', { fg = nil }).fg or astronvim.status.hl.lualine_mode('inactive', HeirlineInsert)
